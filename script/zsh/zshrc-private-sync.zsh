...
# Function to refresh the prompt immediately
refresh-zsh-prompt() {
    # Force the prompt to be redrawn
    if zle; then
        zle .reset-prompt 2>/dev/null
    else
        # If not in zle context, print a newline to trigger prompt refresh
        echo
    fi
}

# Function to check and update prompt if needed
_zshrc_private_check_and_update() {
    # This can be called periodically or after file changes
    if command -v _zshrc_private_update_prompt >/dev/null 2>&1; then
